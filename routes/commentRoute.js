...
router.post('/:commentId/answer', authenticateToken, async (req, res) => {
    ...
    console.log(commentId.commentId, postId.postId);
        ...
        try {
            const result = await executeQuery('UPDATE comments SET isanswer = true WHERE id = $1 AND post_id = $2', [commentId.commentId, postId.postId]);
            try {
...
router.delete('/:commentId', authenticateToken, async (req, res) => {
    const commentId = req.params.commentId;
        ...
        try {
            const result = await executeQuery('DELETE FROM comments WHERE id = $1', [commentId]);
            if (result.rowCount === 0) {
                return res.formatResponse({ message: 'Comment not found' }, 404);
            }
