...
export async function fetchPostsByUsername(username) {
  try {
    const response = await fetch(
      `http://localhost:3000/user/${username}/posts`,
      {
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${localStorage.getItem("token")}`,
...
export async function searchPosts(searchTerm) {
  ...
  const filteredPosts = posts.filter((post) => {
    ...
    const contentMatch = post.content
      .toLowerCase()
      .includes(searchTerm.toLowerCase());
    const subjectMatch = post.subject
      .toLowerCase()
      .includes(searchTerm.toLowerCase());
...
export async function createPostElement(
  post,
  voteData,
  commentCounts,
  userVotes
  ...
  postElement.innerHTML = `
    ...
    commentCounts[post.id] || 0
                      ...
                      ${
                        role === "admin"
                          ? `<button class="button is-danger is-small is-pulled-right" id="deletePost-${post.id}">Delete Post</button>`
                          : ""
                      }
                  </div>
              </div>
