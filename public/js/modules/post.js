    ...
    .then(comments => {
      ...
      comments.forEach(comment => {
        ...
        commentElement.innerHTML = `
            ...
            ${localStorage.getItem('token') && (postUsername === getUserIdFromToken(localStorage.getItem('token')) || getRoleFromToken(localStorage.getItem('token')) === 'admin' || comment.username === getUserIdFromToken(localStorage.getItem('token'))) ? `<button id="deleteCommentButton-${comment.id}" class="button is-danger is-small my-2">Delete Comment</button>` : ''}            ${!comment.isanswer && postUsername === getUserIdFromToken(localStorage.getItem('token')) ? `<button id="MarkCommentAsAnswerButton-${comment.id}" class="button is-success is-small my-2">Mark as Answer</button>` : ''}
